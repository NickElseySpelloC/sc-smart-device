...
output1_identity = "Sydney Dev A O1"
output2_identity = "Sydney Dev A O2"
meter_identity = "Sydney Dev A M1"

# Test a Tasmota switch
# device_identity = "Sydney Dev B"
# output1_identity = "Sydney Dev B O1"
# output2_identity = "Sydney Dev B O2"
...
def test_basic(logger: SCLogger, smart_switch_control: SCSmartDevice) -> None:
    ...
    try:
        ...
        output1_obj = smart_switch_control.get_device_component("output", output1_identity)
        is_online = smart_switch_control.is_device_online(device)
        smart_switch_control.get_device_status(device)
        current_state = output1_obj.get("State", False)  # Default to False if State is not found
        logger.log_message(f"#1 Output status for {output1_identity}: Is Online: {is_online}, Current State: {current_state}", "detailed")
        ...
        smart_switch_control.change_output(output1_identity, not current_state)

        # Get output2 state to verify it is unaffected by output1 change
        output2_obj = smart_switch_control.get_device_component("output", output2_identity)
        output2_state = output2_obj.get("State", False)  # Default to False if State is not found
        logger.log_message(f"Output status for {output2_identity} after changing {output1_identity}: Current State: {output2_state}", "detailed")
        ...
        smart_switch_control.get_device_status(device)
        current_state = output1_obj.get("State", False)  # Default to False if State is not found
        logger.log_message(f"#2 Output status for {output1_identity}: Is Online: {is_online}, Current State: {current_state}", "detailed")
