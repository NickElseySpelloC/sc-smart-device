...
This page provides a quick reference to the attributes of the main SCSmartDevice attributes. 

Note: Please use the [SmartDeviceView class](sc_smart_device_view.md) for a thread-safe, read only copy of these attributes.

## Devices

SCSmartDevice.devices is a list of all the  smart switch devices being managed by this class. You can reference this attribute directly or use SCSmartDevice.get_device() to return a device dict, looking up by ID, name, etc. 

Each device has the following attrbutes

- **Index**: The index of the device in the devices list. Counts from 0.
- **Model**: The model tag, used to look up the device characteristics in the [shelly_models.json](shelly_models_list.md) (if a Shelly device) file~
...
A component is a specific hardware component of a device. Four component types are supported:

- **Inputs**: Shelly devices only. An input usually wiring in from a physical light switch. Used to send a hardware signal to the Shelly device (e.g. light switch has been turned on). 
- **Outputs**: A relay output from the device. This is how the device controls things like lights, motors, etc.
...
- **TempProbes**: Shelly devices only. A ds18b20 digitial temperature probe connected to a Shelly add-on device.
...
**These attributes are retrieved from the [shelly_models.json](shelly_models_list.md) file.
